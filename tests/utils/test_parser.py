...
class TestPredicateParser(unittest.TestCase):
    def test_parse_formula(self):
        ...
        self.assertRaises(NotWellFormed, classical_predicate_parser.parse, '∀x ((P(x) and R(x,y)))')
        f = classical_predicate_parser.parse('∀x ∈ a P(x)')
        self.assertEqual(f, PredicateFormula(['∀', 'x', '∈', 'a', ['P', 'x']]))
        self.assertRaises(NotWellFormed, classical_predicate_parser.parse, '∀x ∈ a (P(x))')
        ...
        self.assertRaises(NotWellFormed, classical_predicate_parser.parse, '∀X (X(a))')
